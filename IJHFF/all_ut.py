...
# plot velocity
plot(y_inc_bot,ut_bot,'o',color='r',label=r'$Robin$',markerfacecolor='none',markeredgecolor='r')
#plot(y_inc_bot,vt_bot,'--',color='r',label=r'$vT$')
#plot(y_inc_bot,tt_bot,'-.',color='r',label=r'$TT$')
plot(y_ref,ut_ref,'x',color='g',label=r'$isoQ$')
#plot(y_ref,vt_ref,'--x',color='g')
#plot(y_ref,tt_ref,'-.x',color='g')
plot(y_refd,ut_refd,'+',color='b',label=r'$isoT$')
#plot(y_refd,vt_refd,'--+',color='b')
#plot(y_refd,tt_refd,'-.+',color='b')
plot(y_cht,ut_cht,'-',color='k',label=r'$Conjug$')

#Graph settings
xscale('log')
yscale('log')
axis([0.3,150,5*(10**-3),7])
xlabel(r"$y^+$")
ylabel(r"$<u'T'>$")
legend(bbox_to_anchor=(0.9,0.7),numpoints=1)

savefig("all_utcht_log.png",bbox_inches='tight')
savefig("all_utcht_log.pdf",bbox_inches='tight')

xscale('linear')
yscale('linear')
savefig("all_utcht_lin.png",bbox_inches='tight')
savefig("all_utcht_lin.pdf",bbox_inches='tight')

xscale('log')
yscale('linear')
savefig("all_utcht_linlog.png",bbox_inches='tight')
savefig("all_utcht_linlog.pdf",bbox_inches='tight')
