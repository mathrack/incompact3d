...
!********************************************************************
!
subroutine corgp (ux,gx,uy,uz,px,py,pz,phi,teta)
! 
!********************************************************************
...
real(mytype),dimension(ysize(1),ysize(2),ysize(3)) :: gx,teta
...
if (itype==2) then !channel flow
   ...
   call transpose_x_to_y(phi,teta)
   call channel(gx,teta)
   call transpose_y_to_x(teta,phi)
...
if (iin.eq.1) then !generation of a random noise
   ...
   call random_seed(put = code+63949*nrank*(/ (i - 1, i = 1, ii) /))
