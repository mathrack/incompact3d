...
if ((ncly==1).or.(ncly==2)) then
   ...
   elseif (npaire==2) then
      ...
      ty(:,1,:)=(uy(:,1,:)+2*uy(:,2,:)+uy(:,3,:))/4. ! alpha = 1
      do j=2,nym-1
         ty(:,j,:)=(uy(:,j+1,:)+uy(:,j  ,:))*imp_aiciy6 &
                  +(uy(:,j+2,:)+uy(:,j-1,:))*imp_biciy6 ! alpha = 3/10
      enddo
      ty(:,nym,:)=(uy(:,ny,:)+2*uy(:,nym,:)+uy(:,nym-1,:))/4. ! alpha = 1
      do j=2,nym
         ty(:,j,:)=ty(:,j,:)-ty(:,j-1,:)*cisy6(j)
      enddo
      ty(:,nym,:)=ty(:,nym,:)*ciwy6(nym)
      do j=nym-1,1,-1
         ty(:,j,:)=(ty(:,j,:)-cify6(j)*ty(:,j+1,:))*ciwy6(j)
      enddo
   endif
endif
...
if ((ncly==1).or.(ncly==2)) then
   ...
   elseif (npaire==2) then
      ! Experimental term for implicit time scheme
      ty(:,1,:)=(-uy(:,1,:)+2*uy(:,2,:)-uy(:,3,:))/dy ! alpha = -1
      ...
      ty(:,ny  ,:)=(-uy(:,ny-1,:)+2*uy(:,ny-2,:)-uy(:,ny-3,:))/dy ! alpha = -1
      do j=2,ny
         ty(:,j,:)=ty(:,j,:)-ty(:,j-1,:)*csi6y(j)
      enddo
      ty(:,ny,:)=ty(:,ny,:)*cwi6y(ny)
      do j=ny-1,1,-1
         ty(:,j,:)=(ty(:,j,:)-cfi6y(j)*ty(:,j+1,:))*cwi6y(j)
      enddo
   endif
endif
