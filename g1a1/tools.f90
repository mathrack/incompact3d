...
use conjugate_ht, only : temp_bot,temp_top,&
                         ny_sol_bot,ny_sol_top,&
                         yp_bot,yp_top,&
                         ly_sol_bot,ly_sol_top,&
                         fluxratio_bot,fluxratio_top
#endif

implicit none

real(mytype),dimension(ysize(1),ysize(2),ysize(3)) :: ux,teta
...
if (.true.) then ! Spatial average of temperature
  ut3=0.
  do k=1,ysize(3)
  do i=1,ysize(1)
    ut=0.
    do j=1,ysize(2)-1
      if (istret.ne.0) ut=ut+(yp(j+1)-yp(j))*(teta(i,j+1,k)-0.5*(teta(i,j+1,k)-teta(i,j,k)))
      if (istret.eq.0) ut=ut+(yly/(ny-1))*(teta(i,j+1,k)-0.5*(teta(i,j+1,k)-teta(i,j,k)))
    enddo
#ifdef my_mod_solide
    do j=1,ny_sol_bot+2
      ut=ut+(yp_bot(j+1)-yp_bot(j))*(temp_bot(i,j+1,k)-0.5*(temp_bot(i,j+1,k)-temp_bot(i,j,k)))
    enddo
    do j=1,ny_sol_top+2
      ut=ut+(yp_top(j+1)-yp_top(j))*(temp_top(i,j+1,k)-0.5*(temp_top(i,j+1,k)-temp_top(i,j,k)))
    enddo
    ut=ut/(yp(ysize(2))-yp(1)+ly_sol_bot+ly_sol_top)
#else
    ut=ut/(yp(ysize(2))-yp(1))
#endif
    ut3=ut3+ut
  ...
  teta=teta-ut4
#ifdef my_mod_solide
  temp_bot=temp_bot-ut4
  temp_top=temp_top-ut4
#endif
