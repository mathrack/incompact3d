...
cd /home/C91471/Documents/Bibliographie/Rapport3A/images/conjug/tmp/raw
...
[yb,phib] = read_my_stat_1d('t_bot_m1d.dat'		,nb_sol);
[yb,phiphib] = read_my_stat_1d('t2_bot_m1d.dat' ,nb_sol);
[yt,phit] = read_my_stat_1d('t_top_m1d.dat'		,nb_sol);
[yt,phiphit] = read_my_stat_1d('t2_top_m1d.dat' ,nb_sol);

write_csv([y,ume,vme,wme,phime], "../csv/raw_1.csv"," ");
write_csv([y,uume,vvme,wwme,uvme,uphime,vphime,phiphime], "../csv/raw_2.csv"," ");
write_csv([yb,phib,phiphib], "../csv/raw_b.csv"," ");
write_csv([yt,phit,phiphit], "../csv/raw_t.csv"," ");
...
ybm=zeros(nb_sol,1);
ytp=zeros(nb_sol,1);
phibm=zeros(nb_sol,1);
phitp=zeros(nb_sol,1);
phiphibm=zeros(nb_sol,1);
phiphitp=zeros(nb_sol,1);
...
for i=1:nb_sol
  ybm(i)=re*utau_m*yb(i);
  phibm(i)=phib(i)/Ttau_m;
  phiphibm(i)=phiphib(i)/(Ttau_m*Ttau_m);

  ytp(i)=re*utau_p*(2-yt(nb_sol-i+1));
  phitp(i)=phit(nb_sol-i+1)/Ttau_p;
  phiphitp(i)=phiphit(nb_sol-i+1)/(Ttau_p*Ttau_p);
end
...
write_csv([ybm,phibm-phibm(nb_sol)], "../csv/moyb.csv"," ");
write_csv([ytp,phitp-phitp(nb_sol)], "../csv/moyt.csv"," ");
write_csv([ybm,phiphibm], "../csv/fluctb.csv"," ");
write_csv([ytp,phiphitp], "../csv/fluctt.csv"," ");
